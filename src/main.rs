...
fn enumerate_processes() -> Option<Vec<Process>> {
    ...
    for entry in fs::read_dir(path).expect("Unable to list") {
        ...
        if entry.path().is_dir() {
            if entry.file_name().into_string().expect("not a interger type path").parse::<u32>().is_ok() {
                let proc = Process {
                    pid: entry.file_name().into_string().expect("not an interger").parse::<u32>().unwrap(), 
                    dir: entry,
                };
                procs.push(proc);
            }
